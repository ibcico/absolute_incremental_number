<?php
	...
	Class fieldabsolute_incremental_number extends Field{

		function __construct(&$parent){
			parent::__construct($parent);
			$this->_name = 'Absolute Incremental Number';
			$this->_required = true;
		...
		public function displayPublishPanel(&$wrapper, $data=NULL, $flagWithError=NULL, $fieldnamePrefix=NULL, $fieldnamePostfix=NULL)
		{
			...
			$label->appendChild(Widget::Input('fields'.$fieldnamePrefix.'['.$this->get('element_name').']'.$fieldnamePostfix, $value, null, array('readonly' => 'readonly')));
		...
		public function getNewNumber()
		{
			...
			$start_number = Symphony::Database()->fetchVar("start_number", 0, "
				SELECT `start_number`
				FROM `tbl_fields_absolute_incremental_number`
				WHERE field_id = $id
			");
			
			$value = Symphony::Database()->fetchVar("value", 0, "
				SELECT `value`
				FROM `tbl_fields_absolute_incremental_number`
				WHERE field_id = $id
			");
			...
			Symphony::Database()->query("UPDATE tbl_fields_absolute_incremental_number SET value = $value WHERE field_id = $id");
			
			return $value + $start_number - 1;
		}
